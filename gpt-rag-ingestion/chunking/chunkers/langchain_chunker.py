    ...
    def get_chunks(self):
        ...
        for text_chunk, num_tokens in text_chunks:
            if num_tokens >= self.minimum_chunk_size:
                ...
                if not short_text_fallback and text_chunk and text_chunk.strip():
                    short_text_fallback = text_chunk.strip()
                skipped_chunks += 1

        # If content exists but all chunks were too small, index one short chunk with real embeddings.
        if not chunks and short_text_fallback:
            fallback_size = self.token_estimator.estimate_tokens(short_text_fallback)
            if fallback_size > self.max_chunk_size:
                short_text_fallback = self._truncate_chunk(short_text_fallback)
            chunks.append(self._create_chunk(0, short_text_fallback))
            logging.info(
                f"[langchain_chunker][{self.filename}] no chunk met minimum size; "
                "indexed one short-text fallback chunk"
            )
