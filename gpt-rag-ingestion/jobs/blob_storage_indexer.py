...
class BlobStorageDocumentIndexer:
            ...
            async for blob in container.list_blobs():
                ...
                try:
                    ...
                    for entry in entries:
                        ...
                        if is_website_file:
                            # Website URLs file: use id as lookup key and title for display.
                            entry_id = entry.get("id", "")
                            if entry_id:
                                display_title = (entry.get("title") or "").strip() or entry_id
                                result[entry_id] = {"title": display_title, "url": url}
                        else:
    ...
    async def _process_one(
        ...
        try:
            ...
            if self._should_stage_excel_rowwise(data):
                ...
                if not all_docs:
                    logging.info(
                        f"[{self.cfg.indexer_name}] No chunks created for {blob_name}; "
                        "uploading metadata-only fallback document."
                    )
                    metadata_chunk = self._build_metadata_only_chunk(
                        blob_name=blob_name,
                        parent_id=parent_id,
                        source_title=_source_title,
                        source_url=_source_url,
    ...
    def _build_metadata_only_chunk(
        self,
        blob_name: str,
        parent_id: str,
        source_title: str = "",
        ...
        """Build a minimal chunk so near-empty files remain discoverable in search."""
        vector_size = int(self._app.get("EMBEDDINGS_VECTOR_DIMENSIONS", "3072"))
        filename = os.path.basename(blob_name)
        title = source_title or os.path.splitext(filename)[0]

        content_parts = [f"Filename: {filename}"]
        if title:
        ...
        return {
            ...
            "relatedImages": [],
            "relatedFiles": [],
            "contentVector": [0.0] * vector_size,
            "captionVector": [0.0] * vector_size,
        }
