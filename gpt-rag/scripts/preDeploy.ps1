...
# Bypass execution policy for this process so azd hook invocation never fails
# due to RemoteSigned / Restricted policy on the local machine.
Set-ExecutionPolicy -Scope Process -Force -ExecutionPolicy Bypass -ErrorAction SilentlyContinue
...
# Consistency check: warn if containerAppsList entries have no corresponding manifest component.
# Config drift occurs when an app is provisioned by Bicep but its deploy script is never run.
$parametersPath = Join-Path $repoRoot 'infra\main.parameters.json'
if (Test-Path -LiteralPath $parametersPath) {
  try {
    $params = Get-Content -Raw -LiteralPath $parametersPath | ConvertFrom-Json
    $deployedServiceNames = @($manifest.components | ForEach-Object {
      # Component repo name (e.g. gpt-rag-orchestration) maps to service_name in containerAppsList
      # by convention: last path segment after stripping "gpt-rag-" prefix.
      ($_.name -replace '^gpt-rag-', '')
    })
    $appListServices = @($params.parameters.containerAppsList.value | ForEach-Object { $_.service_name })
    $orphaned = $appListServices | Where-Object { $_ -notin $deployedServiceNames }
    if ($orphaned) {
      Write-Warning ("DRIFT RISK: The following services are in containerAppsList but have NO " +
        "corresponding manifest component. Their container apps will be provisioned with the " +
        "hello-world placeholder and will NEVER be updated by this deploy hook: " +
        ($orphaned -join ', '))
      Write-Warning ("Add these components back to infra\manifest.json or remove them from " +
        "containerAppsList in infra\main.parameters.json to prevent unhealthy revisions.")
    }
  } catch {
    Write-Warning "Could not parse main.parameters.json for consistency check: $_"
  }
}
