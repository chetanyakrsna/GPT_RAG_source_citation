...
@description('Address prefixes for the virtual network.')
param vnetAddressPrefixes array = [
  '10.20.0.0/21' // 10.20.0.0 – 10.20.7.255 (2048 IPs total)
]

@description('AI Foundry Agents subnet — Recommended /24 (256 IPs)')
param agentSubnetPrefix string = '10.20.0.0/24' // 10.20.0.0–10.20.0.255

@description('Azure Container Apps Environment subnet — /24 (256 IPs)')
param acaEnvironmentSubnetPrefix string = '10.20.1.0/24' // 10.20.1.0–10.20.1.255

@description('Private Endpoints subnet — /26 (64 IPs)')
param peSubnetPrefix string = '10.20.2.0/26' // 10.20.2.0–10.20.2.63

@description('Azure Bastion subnet — Required /26')
param azureBastionSubnetPrefix string = '10.20.2.64/26' // 10.20.2.64–10.20.2.127

@description('Azure Firewall subnet — /26')
param azureFirewallSubnetPrefix string = '10.20.2.128/26' // 10.20.2.128–10.20.2.191

@description('Gateway subnet — Required /26')
param gatewaySubnetPrefix string = '10.20.2.192/26' // 10.20.2.192–10.20.2.255

@description('Application Gateway subnet — /27')
param azureAppGatewaySubnetPrefix string = '10.20.3.0/27' // 10.20.3.0–10.20.3.31

@description('API Management subnet — /27')
param apimSubnetPrefix string = '10.20.3.32/27' // 10.20.3.32–10.20.3.63

@description('Jumpbox subnet — /27')
param jumpboxSubnetPrefix string = '10.20.3.64/27' // 10.20.3.64–10.20.3.95

@description('DevOps Build Agents subnet — /27')
param devopsBuildAgentsSubnetPrefix string = '10.20.3.96/27' // 10.20.3.96–10.20.3.127
...
module aiFoundry 'modules/ai-foundry/main.bicep' = if (deployAiFoundry) {
  ...
  dependsOn: [
    ...
    #disable-next-line BCP321
    (aiFoundryCosmosDBAccountResourceId == '' && deployCosmosDb) ? cosmosDBAccount : null
    #disable-next-line BCP321
...
var varAfCosmosCfgComplete = {
  existingResourceId: aiFoundryCosmosDBAccountResourceId != ''
    ? aiFoundryCosmosDBAccountResourceId
    : (deployCosmosDb ? cosmosDBAccount.outputs.resourceId : null)
...
module containerApps 'br/public:avm/res/app/container-app:0.18.1' = [
  for (app, index) in containerAppsList: if (deployContainerApps) {
    ...
    params: {
      ...
      ingressTargetPort: app.?target_port ?? 80
